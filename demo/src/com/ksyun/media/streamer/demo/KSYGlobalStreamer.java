...
public class KSYGlobalStreamer {
    ...
    private static volatile KSYGlobalStreamer mInstance = null;
    ...
    private GLSurfaceView mCurSurfaceView;
    private List<GLSurfaceView> mViewList;
    ...
    public static KSYGlobalStreamer getInstance() {
        if (mInstance == null) {
            synchronized (KSYGlobalStreamer.class) {
                if (mInstance == null) {
                    mInstance = new KSYGlobalStreamer();
                }
            }
        }
    ...
    /**
     * Set params to KSYGlobalStreamer with {@link KSYStreamerConfig}.
     *
    ...
    public void setDisplayPreview(GLSurfaceView surfaceView) {
        if(mCurSurfaceView == surfaceView) {
            return;
        }
        ...
        if(mViewList.contains(surfaceView)) {
            mGLRender.updateView(surfaceView);
        } else {
            mGLRender.init(surfaceView);
        }

        mGLRender.addListener(mGLRenderListener);
        mCurSurfaceView  = surfaceView;
        mViewList.add(surfaceView);
    }
